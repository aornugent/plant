...
**Δ is a per-model constant, 0.10 m for FF16 and TF24 and 0.025 m for K93**, which buys ~1e-06
consumer error flat across the run.

**It is per-model because a grid of constants gives a stand resolution in proportion to its
height, so Δ has to be sized to the heights a model runs at.** K93 is the witness and it was
found by its blessed values moving: its stand starts at 2 m and reaches 8.5, against 18 for the
other two, so it spends its whole life in the regime where a fixed grid is relatively coarse
(§3's young-canopy row). At 0.10 its offspring production sits 1.8e-03 from the refined answer
where knots tied to the canopy top had it at 5.7e-05; at 0.025 it sits at 7.7e-05, and its
blessed values need no re-blessing at all. Read the rule as Δ ≈ (the canopy a run reaches) / 200.
