...
static double growth_double(double tracked_psi, double height) {
...
// [[Rcpp::export]]
Rcpp::List tf24f_collar_check(double tracked_psi = 1.5, double height = 1.0,
                              double delta = 1e-6) {
  ...
  const double fd_grad = (growth_double(tracked_psi + delta, height) -
                          growth_double(tracked_psi - delta, height)) / (2.0 * delta);
