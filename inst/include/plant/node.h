...
void Node<T,E>::compute_rates(const environment_type& environment,
  ...
  // compresses as the spacing between neighbouring sizes changes. Both cases
  // live in Individual::log_density_rate(), which the cohort block's
  // block_outputs() reads as well: the block exists to reproduce this line, so
  // the coordinate must not be branched on here and not there.
