...
public:
  ...
  // (theta > theta_res) is structural -- no finite zeta maps to or below theta_res --
  // so the state needs no clamp. The R interface stays in theta (set/get_soil_water_state
  ...
  virtual void compute_rates(std::vector<double> const &resource_depletion)
  {
    ...
    double infiltration = infiltration_rate(theta_from_zeta(vars.state(0)));
    ...
    for (size_t i = 0; i < soil_number_of_depths; i++)
    {
      ...
      // Log-depletion state (R-D, #57): dzeta/dt = (dtheta/dt)/(theta - theta_res).
  ...
  void set_soil_water_state(std::vector<double> state) {
    ...
    for (size_t i = 0; i < (vars.state_size); i++) {
      if(i < soil_number_of_depths){
        vars.set_state(i, zeta_from_theta(state[i]));  // R interface is theta; store zeta (R-D #57)
      } else {
        vars.set_state(i, 0);
      }
  }
