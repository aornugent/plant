...
namespace Rcpp {
template <> SEXP wrap(const plant::util::index&);
template <> plant::util::index as(SEXP);
template <> SEXP wrap(const std::vector<plant::util::index>&);
}
